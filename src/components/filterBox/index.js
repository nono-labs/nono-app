...
import CollectionModal from "./items/collection";
import FilterFoot from "./items/filterFoot";
import TraitModal from "./items/trait";

export default function FilterItem(props) {
  const {
    children,
    filterType = "Collection",
    filterData = new Array(9).fill(),
  } = props;
  ...
  const handleOpen = () => {
    console.log("handleOpen,", filterType);
    setVisible({
      ...visible,
      [filterType === "Collection" ? "collection" : "trait"]: true,
    });
  };
  return (
    <>
      <Box className={classes.root}>
        <Hidden xsDown>
          ...
          <Grid className={classes.box}>
            <Typography className={classes.titleLabel}>{filterType}</Typography>
            <Box>
              {filterData.map((_, i) => (
                <Chip
                  label={`Bored Ape ${i}`}
      ...
      {children}
      <Hidden smUp>
        <FilterFoot filterType={filterType} />
      </Hidden>
    </>
  );
}
